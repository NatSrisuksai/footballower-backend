    ...
    try {
        ...
        matchesContainer.find('div.game').each((index, element) => {
            ...
            if (matches.length < 5) {
                ...
                const wrongName = ["Tottenham FC","Brighton FC","Ipswich FC","West Ham FC","Leicester FC"];
                const correctName = ["Tottenham Hotspur FC","Brighton and Hove Albion FC","Ipswich Town FC","West Ham United FC","Leicester City FC"]
                ...
                teamContainer.find('div.col-xs-5').each((index, element) => {
                    if (index < 2){
                        ...
                        if (wrongName.includes(teamName)) {
                            teamName = correctName[wrongName.findIndex((element) => element === teamName)];
                        }
...
//API route to get all team data
app.get("/", cors() ,async (req, res) => {
    try {
      const mergedData = await fetchAndMergeData(); 
      res.json(mergedData); // Send the merged data as JSON response
    } catch (error) {
...
app.post("/register", 
    ...
    try {
        ...
        const insertQuery = "INSERT INTO userdata (username, email, password) VALUES ($1, $2, $3)";
